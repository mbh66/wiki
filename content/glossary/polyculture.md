---
title: "Polyculture"
aliases: []
tags: ["glossary", "agriculture", "regeneration"]
term_type: "borrowed-technical"
first_defined_in: ""
related_terms: ["bioplace", "permaculture", "water-retention-landscape", "regenerative", "extractive"]
source_project: "BioConomy"
created: 2026-09-04
updated: 2026-09-04
epistemic_status: "documented-framework"
---


The cultivation of multiple crop species (and often multiple canopy layers, animals, and functional guilds) together in the same parcel, arranged so that the species support one another and imitate the layered structure of the natural ecosystem the parcel sits inside. Polyculture is the design opposite of monoculture, which grows one crop across a large area to industrial specifications and depends on machinery, chemical inputs, and continued intervention against the land's tendency toward diversity.

Traditional agriculture across most of human history has been polyculture. The Mesoamerican milpa (maize, beans, and squash grown together), the tropical forest garden, the Chinese chinampa, the temperate agroforestry systems of medieval Europe, and the mixed cropping systems of most indigenous food traditions all rely on the same underlying pattern: multiple species interacting produce more total yield, hold soil and water better, resist pests without external inputs, and rebuild rather than deplete the substrate over time.

Polyculture returns to modern agriculture through the [[permaculture|permaculture]] design system and adjacent regenerative practices. At the [[bioplace|BioPlace]] scale it is the working method that turns a parcel into a functional ecosystem rather than a production line for a single crop.

## Related terms

- [[bioplace|BioPlace]]
- [[permaculture|Permaculture]]
- [[water-retention-landscape|Water Retention Landscape]]
- [[regenerative|Regenerative]]
- [[extractive|Extractive]]

## Related pages

- [[concepts/the-bioplace-layer|The BioPlace Layer]]

## Sources and associated figures

- [[mollison-bill|Bill Mollison]]
- [[holzer-sepp|Sepp Holzer]]
- [[perkins-richard|Richard Perkins]]
- [[millison-andrew|Andrew Millison]]

## Provenance

Entry created 4 September 2026 as part of the BioPlace layer buildout. The term is a standard technical borrowing from agroecology and permaculture; the entry places it in the BioConomy corpus as the working method of the BioPlace layer.

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "DefinedTerm",
  "@id": "https://wiki.bioconomy.earth/glossary/polyculture/",
  "name": "Polyculture",
  "description": "The cultivation of multiple crop species (and often multiple canopy layers, animals, and functional guilds) together in the same parcel, arranged so that the species support one another and imitate the layered structure of the natural ecosystem the parcel sits inside. Polyculture is the design opposite of monoculture, which grows one crop across a large area to industrial specifications and depends on machinery, chemical inputs, and continued intervention against the land's tendency toward diversity.",
  "termCode": "polyculture",
  "inDefinedTermSet": {
    "@type": "DefinedTermSet",
    "@id": "https://wiki.bioconomy.earth/glossary/#termset",
    "name": "BioConomy Glossary"
  },
  "url": "https://wiki.bioconomy.earth/glossary/polyculture/"
}
</script>
